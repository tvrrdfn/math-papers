;(function (parent, fun) {
	...
	var THRESH_HOLD = 100,  // 节流函数阈值
		...
		ATTR_ID = 'data-id',
		ATTR_NAME = 'data-name',
		PLACEHOLDER = 'Search';//搜索框中的placeholder
	...
	/**
	 *根据dataType，返回svg的id值
	 */
	function dataTypeToSvgName(dataType){
		var name = '';
		switch(dataType){
			case 'STRING':
				name = 'string';
	...
	function build2Html(treeList, options) {
		...
		for (var i = 0; i < treeList.length; i++) {
			var node = treeList[i],
				...
				hasChildrenClsName = hasChildren ? HAS_CHILDREN_CLASS_NAME : 'item',
			...
			htmlContent += supplant("<li class='{hasChildrenClsName} {activeClsName}' data-last-li='{lastLi}' data-id='{id}' data-name='{name}'>" +
				"<a title='{name}'><svg><use xmlns:xlink='http://www.w3.org/1999/xlink' xlink:href='#icon-ds-"+dataTypeToSvgName(node.dataType)+"'></use></svg><span>{displayName}</span></a>", {
	...
	function renderSearchList(treeList, options) {
		return [
			'<div class="search-list">',
				'<div class="hd">',
					'<input type="text"/><span class="pt-search-auto-focus" data-name="',
					PLACEHOLDER,
					'"></span><a class="pt-search-clear-value"></a>',
				'</div>',
				'<div class="bd">',
	...
	SearchList.prototype = {
		...
		unbindEvent: function() {
			...
			this.element.removeEventListener('input', this.keyup, false);
		},
		click: function (event) {
			//input 点击input中的placeholder，input会获得焦点，放大镜图标完全是装饰作用
			if(event.target && event.target.classList && event.target.classList[0] === 'pt-search-auto-focus'){
				event.target.parentElement.getElementsByTagName('input')[0].focus();
			}else if(event.target && event.target.classList && event.target.classList[0] === 'pt-search-clear-value'){
